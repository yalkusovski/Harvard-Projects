...
def buy():
    ...
    if request.method == "POST":
        symbol = request.get.form("symbol").upper()
        item = lookup(symbol)
        ...
        try:
            ...
            return apology("Insert a number")

        if shares <= 0:
            return render_template("Shares must be positive number")
...
def quote():
    ...
    if request.method == "POST":
        ...
        if not symbol:
          return apology("Symbol is requered")

        item = lookup(symbol)

        if not item:
            return apology("Symbol not supported")
...
def register():
    ...
    if (request.method == "POST"):
        ...
        if not username:
            return apology('username is requered')
        elif not password:
            return apology('password is must')
...
def sell():
    ...
    if request.method == "POST":
        user_id = session["user_id"]
        symbol = request.get.form("symbol")
        shares = int(request.form.get("shares"))
